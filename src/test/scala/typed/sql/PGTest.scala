...
class PGTest extends FunSpec with IOChecker {

  implicit def contextShift: ContextShift[IO] = IO.contextShift(ExecutionContext.global)

  val transactor = Transactor.fromDriverManager[IO](
    "org.postgresql.Driver", "jdbc:postgresql:testdb", "tests", "12345"
  )
  ...
  val create1 = sql"CREATE TABLE IF NOT EXISTS base (a serial primary key, b varchar not null, c varchar not null)"
  create1.update.run.transact(transactor).unsafeRunSync()

  val create2 = sql"CREATE TABLE IF NOT EXISTS test2 (a2 serial primary key)"
...
// TODO: param meta returns BIGINT type for limit and offset
//  it("limit/offset") {
//    check{
